...
void MyJoystickManager::animPerso(float xMove, float yMove)
{
    ...
    irr::core::vector3df position = perso->getPosition();
    irr::core::vector3df rotation = perso->getRotation();

    irr::core::vector3df positionC = cam->getTarget();
    //positionC.normalize();
...
{
    irr::core::vector3df centre = perso->getPosition();
    irr::core::vector3df centreR;
    centreR = perso->getRotation();

    irr::core::vector3df position = cam->getPosition();
    irr::core::vector3df positionRecup = cam->getPosition();
...
void MyJoystickManager::controllerEvent(const irr::SEvent &event)
{
    ...
    if (event.EventType == irr::EET_JOYSTICK_INPUT_EVENT && event.JoystickEvent.Joystick == 1)
    {
        ...
        if (isMoving == 1)
          perso->setMD2Animation(irr::scene::EMAT_RUN);
        if (isMoving == 0)
           perso->setMD2Animation(irr::scene::EMAT_STAND);
